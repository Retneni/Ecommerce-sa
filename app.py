...
from database import db_connect,admin_loginact,user_reg,user_loginact,admin_viewusers,admin_viewrevw,uviewact,uviewdeact,add_categoryact,add_productact,admin_viewproducts,admin_adelete,admin_cate,add_addacountdetailsact,add_moneyact
...
from database import db_connect,purchase1,user_viewcatp,remove1,analyst_loginact,user_viewpurchase,user_rateact,viewstatus
...
@app.route("/pcount.html")
def pcount():
    return render_template("pcount.html")
...
@app.route("/pcountact", methods = ['GET','POST'])
def admin_searchproductsact():
     if request.method == 'POST': 
      product=request.form['product']
      data = admin_search(product)
      print(data)
     return render_template("viewcount.html",search1 = data[0][0])
...
@app.route("/pcount.html")
def pviewproducts():
    data = admin_pviewproducts()
    print(data)
    return render_template("pcount.html",products1 = data)
...
def rateact():
    ...
    if status == 1:
       return render_template("viewpurchasedproducts.html",m1="sucess")
    else:
